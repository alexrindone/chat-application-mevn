  ...
  <b-row>
    <b-col cols="12">
      ...
      <b-list-group class="panel-body" v-chat-scroll>
        <b-list-group-item :key="item.id" v-for="item in chats" class="chat">
          ...
          <div class="right clearfix" v-else>
            <b-img right src="http://placehold.it/50/55C1E7/fff&text=E" rounded="circle" width="75" height="75" alt="img" class="m-1" />
            <div class="chat-body clearfix">
              <div class="header">
                <strong class="primary-font">{{ item.nickname }}</strong> <small class="pull-right text-muted">
                <span class="glyphicon glyphicon-time"></span>{{ item.created_date }}</small>
...
export default {
  ...
  methods: {
    shortner (name) {
      return name.length >= 2 ? name.splice(0, 2) : name
    },
    logout (id) {
